...
public class DialogSpeechRecognizer : MonoBehaviour {
...
//			partialResultText.text = whatToSay;//string.Format("Partial Result: {0}",whatToSay); 
			checkSpeech (whatToSay);
...
//			checkSpeech (whatToSay);
	...
	private void checkSpeech(string saying){
		Text dialog = GameObject.Find ("dialog").GetComponent (typeof(Text)) as Text;
		string temp = dialog.text.ToLower ().Replace(".","").Replace(",","").Replace("!","").Replace("?","");
		if (temp.Equals(saying)) {
